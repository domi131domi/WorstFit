...
phys_clicks i = 0;
phys_clicks how_many;
how_many = mm_in.m1_i1 / (sizeof(phys_clicks) * 2);

current_hole = hole_head;
while( i < how_many && current_hole != NIL_HOLE && current_hole->h_base < swap_base) {
	buffer[i] = current_hole->h_len;
	i++;
	buffer[i] = current_hole->h_base;
	i++;
	current_hole = current_hole->h_next;
}
 buffer[i] = 0;
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *biggest_hole, *before_biggest, *iter;
  ...
  do {
	...
	while (hp != NIL_HOLE && hp->h_base < swap_base) {
		if (hp->h_len >= clicks) {
		/*	printf("z dziury: %d wzieto: %d\n",hp->h_len,clicks);
			iter = hole_head;
			printf("Aktualne dziury: ");*/
		...
		/*	while(iter != NIL_HOLE && iter->h_base < swap_base) {
				printf("%d ",iter->h_len);
				iter = iter->h_next;
			}
			printf("\n");*/
  ...
  do {
	...
	hp = hole_head;
	biggest_hole = hole_head;
	before_biggest = hole_head;
	old_base = hp->h_base;
	while(hp != NIL_HOLE && hp->h_base < swap_base) {
		if(hp->h_len > biggest_hole->h_len) {
			before_biggest = prev_ptr;
			biggest_hole = hp;
		}
		prev_ptr = hp;
		hp = hp->h_next;
	...
	if( biggest_hole->h_len == 0 ) del_slot(before_biggest, biggest_hole);
